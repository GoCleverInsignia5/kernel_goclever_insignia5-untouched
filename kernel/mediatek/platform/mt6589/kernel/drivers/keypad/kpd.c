...
#define GPIO_FCOVER_EINT_PIN GPIO_ALS_EINT_PIN
//78
#define FCOVER_KEY_EINT CUST_EINT_ALS_NUM
// 5
...
//			himax_cover_enable(0);
			schedule_work(&fcover_able);
//======================test=========================
#ifdef KERNEL_OPEN_FCOVER
...
//======================end===========================			
	...
	else  // open
	{
        	...
        	input_sync(kpd_input_dev);
        	printk("report Linux keycode = %u\n", KEY_FCOVER_2);
//			himax_cover_enable(1);
			schedule_work(&fcover_able);
	}
	//printk("\r\n zhaoshaopeng fcover_key_eint_handler in pressed=%d, fcover_key_state=%d,old_state=%d \r\n", pressed, fcover_key_state, old_state);

	/* for detecting the return to old_state */
	mt65xx_eint_set_polarity(FCOVER_KEY_EINT, old_state);
	mt65xx_eint_unmask(FCOVER_KEY_EINT);
}
...
for(j = 0; j < 6; j++) {
 ...
 printk("kpd read addr: 0x%x: data:0x%x\n",addr[j], c);
	...
	mt65xx_eint_set_sens(FCOVER_KEY_EINT, FCOVER_KEY_SENSITIVE);
	mt65xx_eint_set_hw_debounce(FCOVER_KEY_EINT, FCOVER_KEY_DEBOUNCE);
	mt65xx_eint_registration(FCOVER_KEY_EINT, TRUE, FCOVER_KEY_POLARITY,
